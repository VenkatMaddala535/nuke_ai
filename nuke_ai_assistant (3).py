...
class NukeAIWindow(QtWidgets.QWidget):
    """Standalone floating window."""
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(NukeAIWidget(self))
...
def show():
    """Open (or raise) the floating AI panel window."""
    global _window
        ...
        if NUKE_ENV:
            try:
                for w in QtWidgets.QApplication.instance().topLevelWidgets():
                    if "Nuke" in w.windowTitle() and w.isVisible():
                        parent = w
                        break
            except Exception:
                pass
        _window = NukeAIWindow(parent)
    _window.show()
    _window.raise_()
...
#  NUKE GRAPH NODE
#
#  Creates a real NoOp node in the node graph.
#  The node has:
#    • A purple/dark tile so it's visually distinct
#    • A "Open AI Assistant" button knob — click it to open the panel
#    • A note label showing it's the AI node
#    • knobChanged wired so double-clicking the button fires show()
#
#  HOW TO CREATE THE NODE:
#    Run this once in the Script Editor:
#        import nuke_ai_assistant
#        nuke_ai_assistant.create_node()
#
#  After that the node lives in your script. Click the button on it
#  any time to open the AI panel. Save the script and it persists.
#
#  TO AUTO-CREATE ON EVERY SCRIPT OPEN, add to menu.py:
#        import nuke_ai_assistant
#        nuke_ai_assistant.add_to_menu()
#  Then use:  AI Assistant menu → Insert AI Node
# ─────────────────────────────────────────────────────────────────────────────

_AI_NODE_NAME = "NukeAI_Assistant"

# This script is stored inside the node's knobChanged callback
_NODE_KNOB_SCRIPT = """\
import sys, os
_p = os.path.dirname(nuke.thisNode()["script_path"].value())
if _p and _p not in sys.path:
    sys.path.insert(0, _p)
try:
    import nuke_ai_assistant
    if nuke.thisKnob().name() == "open_panel":
        nuke_ai_assistant.show()
except Exception as e:
    nuke.message("Nuke AI: " + str(e) + "\\n\\nMake sure nuke_ai_assistant.py is in:\\n" + _p)
"""


def create_node(script_path=None):
    """
    Create (or select the existing) NukeAI_Assistant node in the node graph.

    Parameters
    ----------
    script_path : str, optional
        Full path to nuke_ai_assistant.py.
        If omitted, the node tries to find it automatically using __file__.
    """
    if not NUKE_ENV:
        print("Not inside Nuke.")
        return None

    # If node already exists, just select and return it
    existing = nuke.toNode(_AI_NODE_NAME)
    if existing:
        nuke.selectAll()
        for n in nuke.allNodes():
            n["selected"].setValue(False)
        existing["selected"].setValue(True)
        nuke.message("NukeAI_Assistant node already exists in this script.\n"
                     "Select it and click 'Open AI Assistant' to launch the panel.")
        return existing

    # Resolve the path to this file so the node can import it
    ...
    # Create a NoOp — it passes through, has no inputs/outputs effect,
    # and is the standard node type used for custom tool nodes in Nuke.
    node = nuke.nodes.NoOp()
    node["name"].setValue(_AI_NODE_NAME)

    # ── Appearance ────────────────────────────────────────────
    node["tile_color"].setValue(0x1a1a2eff)   # dark indigo
    node["gl_color"].setValue(0xc8f046ff)      # lime green outline
    node["label"].setValue(
        "<center><b>◈ Nuke AI</b></center>\n"
        "<center><font color='#888' size='2'>click button below</font></center>"
    )
    node["note_font_size"].setValue(12)

    # ── Custom knobs ──────────────────────────────────────────

    # Divider
    div = nuke.Text_Knob("ai_div", "")
    div.setValue("━━━━━━━━━━━━━━━━━━━━━━━━")
    node.addKnob(div)

    # The launch button — this is what the user clicks
    btn = nuke.PyScript_Knob("open_panel", "  ◈  Open AI Assistant  ", "")
    btn.setFlag(nuke.STARTLINE)
    node.addKnob(btn)

    # Hidden knob that stores the path to nuke_ai_assistant.py
    path_knob = nuke.String_Knob("script_path", "AI Script Path", script_path)
    path_knob.setFlag(nuke.INVISIBLE)
    node.addKnob(path_knob)

    # Info text
    info = nuke.Text_Knob("ai_info", "")
    info.setValue(
        f"<font color='#555' size='1'>"
        f"AI script: {os.path.basename(script_path) if script_path else 'not set'}"
        f"</font>"
    )
    node.addKnob(info)

    # ── Wire up the button's Python script ───────────────────
    # The PyScript_Knob runs this when clicked:
    launch_code = (
        "import sys, os\n"
        "_p = nuke.thisNode()['script_path'].value()\n"
        ...
        "    nuke_ai_assistant.show()\n"
        "except Exception as e:\n"
        "    nuke.message('Nuke AI Error:\\n' + str(e) + "
        "'\\n\\nEnsure nuke_ai_assistant.py is at:\\n' + _p)\n"
    )
    btn.setValue(launch_code)

    # ── Position nicely in the graph ─────────────────────────
    # Place near selected nodes or at origin
    selected = nuke.selectedNodes()
    if selected:
        xs = [n.xpos() for n in selected]
        ys = [n.ypos() for n in selected]
        node.setXYpos(int(sum(xs)/len(xs)) + 200, int(sum(ys)/len(ys)))
    else:
        node.setXYpos(0, -200)

    nuke.message(
        "✅  NukeAI_Assistant node created!\n\n"
        "You'll see it in your node graph.\n"
        "Click  'Open AI Assistant'  on the node to launch the panel.\n\n"
        f"Script path stored: {script_path}"
    )
    return node
...
def add_to_menu():
    """
    Add Nuke menu entries.  Call from ~/.nuke/menu.py:
    ...
    m.addCommand(
        "Insert AI Node into Graph",
        "import nuke_ai_assistant; nuke_ai_assistant.create_node()",
    )


# ─────────────────────────────────────────────────────────────────────────────
#  ENTRY POINT
#  Running from Script Editor:  just paste and run — node gets created.
# ─────────────────────────────────────────────────────────────────────────────

if NUKE_ENV:
    # When executed directly from Nuke's Script Editor,
    # create the node in the graph AND open the panel.
    create_node()
    show()
