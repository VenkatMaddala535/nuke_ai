...
def _open_panel():
    """Open or raise the AI panel. Called by the node button and on first run."""
    global _window
...
# ─────────────────────────────────────────────────────────────────────────────
#  NODE CREATOR
#  Drops a NukeAI_Assistant NoOp into the node graph.
#  Button on the node opens the AI panel.
#  The full launch code is baked into the button — no imports needed later.
# ─────────────────────────────────────────────────────────────────────────────

def _create_ai_node():
    NODE_NAME = "NukeAI_Assistant"

    # Already exists — just re-open the panel, don't make a duplicate
    ...
    node["tile_color"].setValue(0x1a1a2eff)   # dark indigo tile
    node["gl_color"].setValue(0xc8f046ff)      # lime outline
    node["label"].setValue(
        "<center><b><font color='#c8f046'>◈ Nuke AI</font></b></center>"
        "<center><font color='#555' size='2'>double-click → Open</font></center>"
    )
    node["note_font_size"].setValue(11)

    # Store the path to this script file inside the node (invisible knob).
    # This lets the button reload it after a Nuke restart.
    try:
        _this_file = os.path.abspath(__file__)
    except NameError:
        _this_file = ""   # running via exec() — user will need to paste again after restart

    src_knob = nuke.String_Knob("ai_source", "Script path", _this_file)
    src_knob.setFlag(nuke.INVISIBLE)
    node.addKnob(src_knob)

    # Separator
    node.addKnob(nuke.Text_Knob("ai_sep", "", ""))

    # The button — code baked directly inside it
    btn_code = (
        "import nuke, sys, os\n"
        "_src = nuke.thisNode()['ai_source'].value()\n"
        "if _src and os.path.exists(_src):\n"
        "    _d = os.path.dirname(_src)\n"
        "    if _d not in sys.path: sys.path.insert(0, _d)\n"
        "    exec(open(_src).read(), {'__file__': _src})\n"
        "else:\n"
        "    try:\n"
        "        _open_panel()\n"
        "    except NameError:\n"
        "        nuke.message('Paste and run nuke_ai_assistant.py in the Script Editor first.')\n"
    )
    btn = nuke.PyScript_Knob("open_ai", "  ◈  Open AI Assistant  ", btn_code)
    btn.setFlag(nuke.STARTLINE)
    node.addKnob(btn)

    # Place it out of the way, top-left of graph
...
# ─────────────────────────────────────────────────────────────────────────────
#  ENTRY POINT
#  Paste the whole file into Nuke's Script Editor and press Ctrl+Enter (Run).
#  → A NukeAI_Assistant node appears in the graph.
#  → The AI panel opens immediately.
#  Next time: just click the button on the node.
# ─────────────────────────────────────────────────────────────────────────────
