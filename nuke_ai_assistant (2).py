...
# ─────────────────────────────────────────────────────────────────────────────
#  FLOATING WINDOW  (simpler, always works)
# ─────────────────────────────────────────────────────────────────────────────

class NukeAIWindow(QtWidgets.QWidget):
    """Standalone floating window — simplest way to open the assistant."""
...
def show():
    """
    Open (or raise) the floating Nuke AI Assistant window.
    Call this from the Script Editor or menu.py.
    """
    global _window
    if _window is None or not _window.isVisible():
        # Parent to Nuke's main window if possible
        parent = None
        if NUKE_ENV:
            try:
...
def add_to_menu():
    """
    Call from menu.py to add a permanent menu entry:
    ...
    if NUKE_ENV:
        ...
        m.addCommand("Open Panel", "nuke_ai_assistant.show()", "ctrl+shift+a")


# Auto-launch when run directly from Nuke's Script Editor
if __name__ == "__main__":
    show()
else:
    # If imported (import nuke_ai_assistant), also open automatically
    show()
